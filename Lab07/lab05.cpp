//(실습3)아래 코드를 분석하고 실행해 보세요.결과를 확인해 보세요.
...
int main() {
	...
	Manager** managerlist = new Manager*[2];	//2중포인터를 이용, 메모리할당만, 생성자는 아직 호출 안함
	//new로 하면 메모리를 동적으로 할당
	//new로 하면 다른 함수에서 실행되어 함수가 끝나도
	//끝난 후에 메모리에 아직 남아있다, 삭제되지 않음
	Intern** internlist = new Intern*[2];
	Janitor** janitorlist = new Janitor*[2];
	managerlist[0] = new Manager(200, "James", 33);	//이제 생성자 호출
